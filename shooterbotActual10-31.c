...
task Pid1() {
	...
	while (true) {
		...
		monitoredCurrentPosR = currentPos;
		actualSpeed = (currentPos - prevPos);
		monitoredActualSpeedR = actualSpeed;
		speedError = (desiredSpeed - actualSpeed);
		intError += speedError;
		difError = prevError - speedError;
		// motor speed is 75 when motorSpeed = 127
		motorSpeed = desiredSpeed + (speedError * kp / kpden) + (intError * ki / kiden) + (difError * kd / kdden);
		//motorSpeed = 127;
		prevPos = currentPos;
		...
		monitoredMotorSpeedR = motorSpeed;
