...
#pragma config(Motor,  port2,           catapultLeft,  tmotorVex393_MC29, openLoop, reversed, driveLeft)
...
// drive in direction turn at speed for t milliseconds
...
// Drive until both bumper switches are activated by low goal bar or t msec
void driveIntoWall(int t, int speed, int turn)
{
	int tim;
	do (tim = 0; tim < t; tim += 20)
		{
			if(SensorValue[leftBumper] != 0 && SensorValue[rightBumper] != 0) break;
			motor[frontLeft]  =  linearize(speed + (maxSteer*turn/100));
			motor[backLeft]   = -linearize(speed + (maxSteer*turn/100));
			motor[frontRight] = -linearize(speed - (maxSteer*turn/100));
			motor[backRight]  =  linearize(speed - (maxSteer*turn/100));
