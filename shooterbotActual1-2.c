...
#pragma config(Sensor, dgtl3,  ballAtShooter,  sensorTouch)
#pragma config(Sensor, I2C_1,  ,               sensorQuadEncoderOnI2CPort,    , AutoAssign)
#pragma config(Sensor, I2C_2,  ,               sensorQuadEncoderOnI2CPort,    , AutoAssign)
#pragma config(Motor,  port1,           gate,          tmotorVex393_HBridge, openLoop)
#pragma config(Motor,  port2,           intakeLower2,  tmotorVex393_MC29, openLoop, reversed)
...
// 2015-Dec-19   added shooter ball detect switch for speed sense
...
int const maxSteer = 80;  // percent of drive to apply to steering
// int const maxSteer = 50;  // percent of drive to apply to steering 12/19
...
int actualSpeedR = 0;
int shooterCommandL = 0;
int shooterCommandR = 0;
...
int speedTolerance = 80;    // how much speed can be wrong and let it shoot
...
int upperEn = 1;    // true if upper hopper allowed to move up
...
void drive(int t, int speed, int turn) // to drive robot during autonomous
{
	// no problem here :D
...
void intake(int time, int speed)  // move balls in autonomous mode
{
	motor[intakeLower] = speed;
	motor[intakeLower2] = speed;
	motor[intakeUpper] = speed;
	sleep(time); // good night robot :D
}

void shoot(int time, int goalSpeed) // to shoot stuff during autonomous
{
	int t = 0;
	while (t < time)
	{
...
//		motorSpeed = desiredSpeed + (speedError * kp / kpden) + (difError * kd / kdden) + (intError * ki / kiden) ;
		...
		shooterCommandR = motorSpeed; // for debugging
...
//		motorSpeed = desiredSpeed + (speedError * kp / kpden) + (difError * kd / kdden) + (intError * ki / kiden) ;
		...
		shooterCommandL = motorSpeed; // for debugging
...
int recoveryTime = 800;   // milliseconds between shots
// Task for the autonomous portion of the competition.
task autonomous()
{  // first argument is time in milliseconds
	startTask(Pid1);
	startTask(Pid2);
	drive(1, 100, 0); // start to drive forward from starting tile
	shoot(4000, maxLauncherSpeed); // spin up le shooter
	drive(1, 0, 0); // stop driving
	intake(500, -127); // move ball in2 shooter
	intake(recoveryTime, 0); // wait for shooter 2 restore speed
	intake(500, -127); // move ball in2 shooter
	intake(recoveryTime, 0); // wait for shooter 2 restore speed
	intake(500, -127); // move ball in2 shooter
	intake(recoveryTime, 0); // wait for shooter 2 restore speed
	intake(500, -127); // move ball in2 shooter
	intake(recoveryTime, 0); // wait for shooter 2 restore speed
	shoot(5000, 0); // tells shooter 2 stop
}
...
task usercontrol()
{
	...
	while (true)
	{
		...
		// Compute whether the shooter is up to speed
		upperEn = 0;  // assume not allowed to move ball up more
		if ((abs(actualSpeedL - maxLauncherSpeed) < speedTolerance) &&
		    (abs(actualSpeedR - maxLauncherSpeed) < speedTolerance))
			upperEn = 1;
		if (SensorValue[ballAtShooter] == 0)  // No ball present, allow hopper to move up
			upperEn = 1;
		...
		if ((vexRT[Ch3Xmtr2] > 50) && (upperEn == 1)) {
			upperSpeed = -127;  // upper intake runs in when operator left joystick up
		} else if (vexRT[Ch3Xmtr2] < -50) {
			upperSpeed = 127;  // lower intake runs out when operator left joystick down
		}
